...
#######################################################################################################
CAM_ord <- function(y_obser,                         # Observations, organized into
  ...
  for(sim in 2:(nsim*thinning + burn_in)){
    ...
    # zj <- reset(old_zj)
    zj.pg <- rep(zj, nj)
    # theta_per_obs <- theta[cij,]
    # theta         <- unique(theta_per_obs)
    # cij           <- 
    #   apply( outer(theta_per_obs[,1], theta[,1], "-"), 1, function(x) which(x == 0));
    # 
    # ################################################################
    # # Update Distributional Labels
    # omega <- as.matrix(omega[unique(oldcij),])
    # 
    ################################################################
