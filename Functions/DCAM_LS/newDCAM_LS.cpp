...
// [[Rcpp::export]]
arma::colvec vec_log_Likelihood(arma::colvec yobs, arma::rowvec theta, double gamma){
  int n = yobs.n_rows;
  arma::colvec R(n);
  
  for(int i =0; i<n; i++ ){
    R[i] = log_Likelihood(yobs[i], theta, gamma);
  } 
  ...
  for(int i=0; i<N; i++){
    for(int k=0; k<NN_c; k++){
      ...
      p[k] = 
        ...
        log_Likelihood( y_obser[i],  theta.row(k), gamma[i]) - 
        log(xi_c[k]);    
    }
...
///////////////////////////////////////////////////////////// Update theta
// [[Rcpp::export]]
arma::mat Update_theta(arma::colvec y_LAT_overgamma,
                       arma::colvec cij,
  ...
  for(int i = 0; i<NN_c; i++ ){
    ...
    arma::colvec YYY = y_LAT_overgamma.elem(ind);
...
arma::colvec Update_latentY(arma::colvec y_obser,  
  ...
  for(int i=0; i<N; i++){
    current_mu    = theta(cij_cpp[i],0)      *gamma[i];
    current_sigma = sqrt(theta(cij_cpp[i],1))*gamma[i]; 
