...
DCAM_LibSize <- function(y_obser,                         # Observations, organized into
  ...
  for(sim in 2:(nsim*thinning + burn_in)){
    ...
    ################################################################
    # Update Atoms
    theta <- Update_theta(y_LAT_overgamma = y_lat/gamma,
                          cij = cij,
                          a0 = a0,
                          b0 = b0,
                          k0 = k0,
  ...
  if(cheap){
    out <- list(Z_j=Z_j, Csi_ij=Csi_ij,
                A_DP=ALPHA_DP, B_DP=BETA_DP,NN.cz=NN.cz,gamma=gamma,
                y_obser=y_obser, y_group=y_group, nsim=nsim, prior = prior,Sigma.beta=Sigma.beta)
  }else{
    out <- list(Z_j=Z_j, Csi_ij=Csi_ij, PI=PI, THETA=THETA, gamma=gamma,
