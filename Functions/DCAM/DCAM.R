...
DCAM <- function(y_obser,                 # Observations, organized in
  ...
  for(sim in 2:(NSIM*thinning + burn_in)){
    ...
    #   if( runif(1)<.5){
    #     ZA1 <- label_switching_move1(z = zj,pi.z = pi.z,NN.z = NN.z)
    #     zj  <- ZA1[[1]]; acc1[sim] <- ZA1[[2]]
    # #  }else{
    #   ZA2 <- label_switching_move2(z = zj,v.z = v.z,NN.z = NN.z)
    #   zj  <- ZA2[[1]]; acc2[sim] <- ZA2[[2]]
    #  # }
    #   }
    ######################################################
    tsl0.tmp <- UPD_tsl0_for_cij(y_obser = y_obser,Uij = Uij,xi_c = xi.c,omega = omega,zj_pg = zj.pg,tsl0 = tsl0,N = N,NN_c = NN.c)
