...
double log_Likelihood(double yobs, arma::rowvec theta){
  ...
  double current_sigma = sqrt(theta[1]), a1,a2;
  ...
  for(int i=0; i<N; i++){
    for(int k=0; k<NN_c; k++){
      p[k] = log(xi_c[k] > Uij[i]) + log(omega(k,zj_pg[i]-1)) +  
        log_Likelihood( y_obser[i],  tsl0.row(k)) - log(xi_c[k]);    
    }
...
arma::colvec Upd_LAT(arma::colvec y_obser,  
  ...
  for(int i=0; i<N; i++){
    current_mu = tsl0(cij_cpp[i],0);
    current_sigma = sqrt(tsl0(cij_cpp[i],1)); 
